...
std::ostream& printUsage(std::ostream& out) {
  return out
         << R"""(usage: task1 [-b <max-bounces>] [-t [<num-threads>]|--num-threads <num-threads>] [--test-intersections] [--test-shading] <config-file>
...
int taskMain(int argc, char* argv[]) {
  ...
  if (mode) {
    if (mode & 1U)
      task.renderIntersectionTest((output_dir / input_file.filename())
                                      .replace_extension(".intersections.pfm"),
                                  res_x, res_y, num_threads);

    if (mode & 2U)
      task.renderShadingTest((output_dir / input_file.filename())
                                 .replace_extension(".shading.pfm"),
                             res_x, res_y, num_threads);
  } else
