...
class Scene {
  ...
  bool intersectsCone(const float3& p, const float3& d, float t_min,
                      float t_max) const;
  std::tuple<const triangle_t*, int> findClosestHitTriangle(
  ...
  HitPoint hit(const Cone& cone, const float3& p, const float3& d,
               float t) const;
  HitPoint hit(const triangle_t& tri, int material_id, float lambda_1,
               float lambda_2) const;
 ...
 public:
  Scene(std::unique_ptr<float3[]>&& positions,
        ...
        /*std::unique_ptr<triangle_t[]>&& triangles, std::unique_ptr<int[]>&&
